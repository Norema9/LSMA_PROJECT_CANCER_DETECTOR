...
def main(processed_direct, dataset_directory, axial_dir, corona_dir, sagital_dir):
    ...
    hog = HOG()
    lbp_desc = LocalBinaryPatterns(numPoints=24, radius=8)
    lab_histogram = LabHistogram()
    color_channel_statistic = ColorChannelStatistic()

    new_train = {"color": [], "lab": [], "lbp": [], "hog": [], "resnet": [], "lab_hist_path": [], "hog_img_path": [], "lbp_hist_path": [], "lbp_img_path": []}
    ...
    for index, row in tqdm(df_train.iterrows()):
        ...
        lbp_features, lbp = lbp_desc.describe(image_path)  # Only taking the histogram part
        ...
        plt.title(f"LBP Histogram of {filename}")
        plt.xlabel("Features")
        plt.ylabel("# of Pixels")

        save_dir_lbp = os.path.join(processed_direct, "lbp_images", row["label"], row["plan"])
        os.makedirs(save_dir_lbp, exist_ok=True)
        lbp_img_path = os.path.join(save_dir_lbp, f"lbp_image_{filename}.png")
        plt.savefig(lbp_img_path)
        plt.close()
        new_train["lbp_img_path"].append(lbp_img_path)

        (hist, _) = np.histogram(lbp.ravel(), bins=range(0, 256))
        plt.figure()
        plt.plot(hist)
        plt.title(f"Histogram for LBP picture of {filename}")

        save_dir_lbp_hist = os.path.join(processed_direct, "lbp_hist", row["label"], row["plan"])
        os.makedirs(save_dir_lbp_hist, exist_ok=True)
        lbp_hist_path = os.path.join(save_dir_lbp_hist, f"lbp_hist_{filename}.png")
        plt.savefig(lbp_hist_path)
        plt.close()
        new_train["lbp_hist_path"].append(lbp_hist_path)
    ...
    new_test = {"color": [], "lab": [], "lbp": [], "hog": [], "resnet": [], "lab_hist_path": [], "hog_img_path": [], "lbp_hist_path": [], "lbp_img_path": []}
    ...
    for index, row in tqdm(df_test.iterrows()):
        ...
        plt.savefig(lbp_img_path)
        plt.close()
        new_test["lbp_img_path"].append(lbp_img_path)
        ...
        plt.savefig(lbp_hist_path)
        plt.close()
        new_test["lbp_hist_path"].append(lbp_hist_path)
    ...
    lbp_df_train["lbp_img_path"] = new_train["lbp_img_path"]
    lbp_df_train["lbp_hist_path"] = new_train["lbp_hist_path"]
    ...
    hog_df_test["lbp_img_path"] = new_test["lbp_img_path"]
    hog_df_test["lbp_hist_path"] = new_test["lbp_hist_path"]
